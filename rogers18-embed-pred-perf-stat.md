...
* Each year dozens of new models are proposed, each of them
  with multiple hyper-parameters that may dramatically influence performance
...
* implemented in LDT (Linguistic Diagnostics Toolkit),
  an open-source Python library that offers
  a wide range of analysis options with
  corpus-based statistics, psychological association norms, and dictionaries
  * broad lexical coverage thanks to
    a combination of the English WordNet, Wiktionary, and BabelNet
  * potentially extensible to many languages
...
* Crucially, all these approaches assume a single feature of a representation
  ...
  * we: different NLP tasks may rely on its different aspects
...
* LD analysis starts with sampling the corpus vocabulary, see Section 4.2
  ...
  * Each neighbor undergoes spelling normalization
...
* ling relations (especially semantic rels such as hypernymy and antonymy)
...
* Following Baroni and Lenci (2011), we control for the polysemy of the words
  ...
  * Polysemy was defined as a word having over 2 meanings in WordNet 6
* Table 3
* we also exclude the words belonging to several parts of speech
  (eg a dog (noun), to dog (verb)) to preserve the morphological class variable
  * discards a lot of high-frequency vocabulary
    => the higher frequency bins for verbs and adjectives not populated fully
...
* most words will not have 1,000 meaningful relations,
  ...
  * For example, a SG model with bound DEPS context has _rather_ as the
    neighbor of _quite_ at rank 920
...
* 3 tasks relying on how the word embeddings
  ...
  * sentence-level sentiment polarity classification (Sentiment (sent.)) task
    ...
    * Binary classification is performed by
      a simplified version of the model proposed by Kim (2014)
...
* analogy task: we use BATS dataset (Gladkova+ 2016), which is
...
* quantification
  ...
  * Continuous variables are broken down into bins, the
    size of which is chosen empirically: e.g. frequency
    * number of low-frequency or high-frequency neighbors
...
* extrinsic tasks
  ...
  * nL, all the non-sequence-labeling tasks correlate with the intrinsic data
    * the traditional intrinsic tasks are after all useful
  * hE they offer no explanation about why this is the case
...
* The semantic factors that appear to be the least useful across all tasks are
  ...
  * The latter is surprising in the light of such tasks as SNLI that seems to
    clearly rely on it
...
* It could be expected that, in the sample of general English vocabulary, the
  neighbors that are proper nouns or foreign words would be detrimental to any
...
* distributional factors
  ...
  * NonCooccurring factor is apparently useful for
    sequence labeling and some intrinsic tasks to
...
* As a brief demonstration of explanatory power of LD methodology, let us
* CBOW, SG, and GloVe models and their performance on the 8 above tasks in
...
* Table 4 lists some of the
  ...
  * SG that is always slightly ahead in semantic, morphological and distri LD
  ...
  * GloVe vs Word2Vec: amount of LowFreqNeighbors and NonCooccurring words
    ...
    * success of SG is due to its ability to bring together related words
      even if they were rare, and/or did not co-occur in the corpus. This
    ...
    * specialized information is necessary but not sufficient for top perform,
