...
* present SumeCzech, a Czech news-based summarization dataset
...
* We evaluate several baselines for all selected summarization settings
  ...
  * two supervised methods:
    * extractive one inspired by Kupiec+ (1995), and an
...
* Czech
  * Czech part of the MultiLing dataset (Giannakopoulos+ 2015; Li+ 2013;
    Elhadad+ 2013) containing 40 Wikipedia articles, and
  * SummEC (Rott and Červa, 2013) containing 50 news articles
...
* Initially, five Czech news websites were selected to create the dataset:
  ...
  * ceskenoviny.cz, which provides mostly high-quality articles, was added

## 3.2 Data Preparation

* Dumps of the relevant websites’ pages from 10 Common Crawl collections
...
* Language recognition was performed with langdetect
* some documents was dropped based on the headline and/or abstract text
  ...
  * some abstracts were disclaimers that the page belongs to a culinary series
* some documents was dropped based on the presence of certain keywords
...
* duplicate headlines, duplicate abstracts or duplicate texts, only one document
  was retained. Therefore, headlines in the dataset are unique
...
* we wanted
  ...
  * not? to end up with all the documents from one domain in the same part
    * it would introduce even stronger bias to the evaluation
    * but it can be thought of as a common real-life situation
...
* we propose to evaluate summarization methods trained on the SumeCzech dataset
  using full-length F1-score of a language-agnostic variant of ROUGE, which
...
* inspired by the work of Kupiec+ (1995)
...
* two classification algorithms: logistic regression and random forests
...
* abstract→headline setting in Table 4
  * extractive methods perform slightly worse than the `first` baseline, but
...
* text→abstract summarization is evaluated in Table 6, yielding
  * results similar to the previous setup
  * `first` baseline is performing the best, followed by the textrank approach
  * t2t abstractive summarization is the lowest, inferior even to the rand
* In order to compare quality of documents from different websites, we also
  ...
  separately. The results are presented in Table 7
