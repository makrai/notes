...
* 1.3 million sarcastic statements — 10 times more than any previous dataset —
  ...
  * allowing for learning in both balanced and unbalanced label regimes
...
* Reddit comments from December 2005 have been made available
...
* we use the raw corpus described in Section 3
* we construct several useful benchmarks for the task of classification
* All benchmarks provide the full conversation thread leading up to the target
  * along with comment metadata
...
* In the most general case, we use the provided raw files
...
* one sarcastic and one non-sarcastic response 
  from each set of responses to a comment sequence
* task: which of two statements is sarcastic
...
* increases the false negative rate as
  ...
  themselves may not be labeled
* to reduce this issue when picking the nonsarcastic statement, we featurize all
  statements using the normalized sum of Common Crawl GloVe embeddings
* we pick from only those non-sarcastic statements that have similarity ≤ 0.95
  with the sarcastic statement

### 5.1.2 Politics

* The difficulty of detecting sarcasm rests not only on the context 
...
* Even humans will struggle with sarcastic comments drawn from unfamiliar
  topics, for instance, obscure hobbies or art forms
* we also test human and machine performance on comments drawn solely from the
  politics subreddit, 
  * a topic for which all evaluators had sufficient background information
* 17 thousand sequences
