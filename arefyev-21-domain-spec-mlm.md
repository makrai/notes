NB-MLM: Efficient Domain Adaptation of Masked LMs for Sentiment Analysis
...
* the standard MLM objective leads to inefficiency for adaptation step because
  * mostly learns to predict the most frequent words
* We propose a technique for more efficient adaptation that
  ...
  trained for the task at hand, which are likely more relevant
...
* called the domain or task adaptation, depending on the degree of similarity
  of the data for adaptation to the target dataset
* We evaluate the proposed method on two datasets for sentiment analysis
  ...
  * In particular, we consider the task of classifying the binary sentiment
    polarity of a given review
...
* same words masked in each example at each epoch
  * For domain adaptation, only one epoch of training is done on a large
    dataset, hence, smaller temperatures perform better
