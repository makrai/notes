...
* we quantify the semantic difference between public Tweets and private mental
  health journals used in online cognitive behavioral therapy. We will use 
  ...
  * for the task of emotional valence prediction, social media can be
    ...
    mental health models. Our results suggest that 
* the semantic gap between public and private self-expression is small, and that
...
* we deal strictly with internal sentiment analysis, specifically with the
  ...
  * partly aligns with emotion detection in text 7–12 , a subtask of the field
  * we focus on the expansion of valence categories in a mental health setting.  
* online mental health therapy services 13–23 . These programs provide the
  ...
  * users typically submit several directed journals 
    ...
    * system for automatically categorizing user text has obvious benefits to
      * early warning for suicide risk, providing a positive and
        always-available feedback for patients with distorted thinking, or
        simply providing enhanced and more fine-grained analysis of overall
        patient well-being.
...
* for mental health polarity. Rather than framing the subjectivity
  * besides positivity and negativity, we introduce two additional classes of
    polarity: both positive and negative, and neither positive nor negative. We
  * psychological research, which suggests emotions cannot be represented on a
    single axis of valence 24–28 .
...
* publicly-available mental health datasets suitable for machine learning-based
  internal sentiment analysis are few and far between. However, large amounts of
  * traditional sentiment analysis in the context of social media platforms such
    as Twitter ?, 29–34 . Given social media users’ tendency
...
* We quantify the similarity between the two domains using transfer learning.
  ...
  * fine-tuning them on another target domain (i.e.  mental health text). We
  * in the context of deep learning, a subfield of machine learning 
* We train deep learning models on a large dataset of annotated text from the
  ...
  * objective change in mental health 
    as gauged by evidence-based mental health measures. We demonstrate that
* improved performance for emotional valence prediction, and argue that social

# Methods 3

## Transfer learning

* Our target domain: text-based emotional self-expressions and reflections from
  a cognitive-behavioral perspective, obtained through an online mental health
  therapy service
...
* important implications for computer-assisted, computerized psychotherapy, and
  ...
  * negative affect in patient narratives suggesting elevated levels of
    distress, depression, or anxiety. It can also be used to detect cases in
  * when positive and negative affect are both high in narrative text, which may
    * ambivalence, which is often associated with conflict-related distress. In
...
* patients might be skeptical of the accuracy of therapist feedback or reactive
  to the fact that a healthcare provider is providing feedback, those same
  * might place their trust in the more objective feedback from the ML tool. In
  * there is evidence that ostensibly objective personality assessment feedback
    is widely accepted, especially if it is worded somewhat positively and
    somewhat generally 
