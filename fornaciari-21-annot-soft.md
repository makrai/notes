...
* the MTL framework allows us to indirectly evaluate its effect on the main
  ...
  the effect of different loss functions for the soft label task
...
* The KL divergence is a natural choice to measure the difference between
  ...
  * standard KL-divergence is: `D_KL (P ‖ Q) = \sum Pi log2 Pi/Qi`
    * encourages a wide Q, because if the model overestimates the regions of
      small mass from P it will be heavily penalised
  * inverse KL-divergence is: `D_KL (Q ‖ P)`
    * This measures the divergence from P to Q and
      encourages a narrow Q distribution because the model will try to
      allocate mass to Q in all the places where P has mass; 
      otherwise, it will get a strong penalty
...
* A third option is to directly apply Cross-Entropy
  * derived from KL-divergence, the entropy of P added to the KL-divergence: ␒
    H(P ‖ Q) = H(P) + \sum Pi log2 Pi/Qi
  * regular KL-divergence and CrossEntropy tend to lead to the
    same performance
  * For completeness, we report the results of CrossEntropy as well
* overall loss of the main and of the auxiliary task: sum
  * no normalization method to the two losses, as unnecessary
  * LogSoftmax activation function for the main task, which is
    a standard choice for one-hot encoded labels, and 
  * auxiliary
    * standard Softmax for the auxiliary task. 
      * Against the distributions of gold (one-hot encoded) and soft labels,
        both summing up to one, the errors are on the same scale
...
* Since we did not create the corpora that we use in our experiments, 
  we do not know the details of the gold labels’ creation process
...
* ~> the MTL models exploit an additional source of information
...
* We ran a set of experiments where the main task was trained on the majority
  ...
  * test still performed on the gold labels
  * both tasks rely on the same source of (imperfect) information
* tab 2: overall performance drops compared to the results of Table 1, Table 2
* the MTL models still maintain a significant advantage over the STL ones

# 5 Error analysis

* To gain further insights about their contributions,
  we inspect the soft labels’ probability distributions,
  comparing the predictions of STL and MTL models
...
* For each of these categories, we compute the
  relative kurtosis of the soft labels
  * kurtosis describes how uniform the probability distribution is: whether 
    * the annotators agree on a single class, or whether 
    * they disperse their votes among different classes
* results
...
* More interesting is the outcome where 
  ...
  * the average kurtosis lies between the two previous extremes
  ...
  * To measure the significance of this trend, we apply the
    Mann-Whitney rank test (Mann and Whitney, 1947). We use a 
    non-parametric test because the kurtoses’ distribution is not normal
...
* aggregation of multiple annotations before model training
  * Dawid and Skene (1979) proposed an
    Expectation-Maximization (EM) based aggregation model 
...
* filtering out presumably low quality data to train on the remaining data
  (Beigman Klebanov and Beigman, 2014; Jamison and Gurevych, 2015)
  * requires an effective filtering threshold, which is non-trivial;
    relying only on high-agreement cases also results in worse performance
    (Jamison and Gurevych, 2015)
  ...
  * the robustness of machine learning models is affected by the data
    ...
    performance
* The closest to our work are the studies of 
  * Cohn and Specia (2013) and Rodrigues and Pereira (2018), who both use MTL
    * each of their tasks represents an annotator
    * We instead propose to learn from both the gold labels and the distribution
      over multiple annotators, which we treat as soft label distributions in a
      single auxiliary task
    * our approach requires fewer output nodes
  * the only study that directly uses soft labels is the one by Lalor+ (2017)
    ...
    * they use them to fine-tune STL networks
...
* label smoothing (Pereyra+ 2017) penalizes the cases of over-confident
  network predictions
  * Both label smoothing and soft labels
    reduce overfitting regulating the loss size
...
* annotator biases (Shah+ 2020; Sap+ 2019; Hovy and Yang, 2021)

# 7 Conclusion

* We propose a new method for leveraging instance ambiguity, as 
  * ambiguity expressed by the probability distribution over label annotations.
  * We set up MTL models to predict this label distribution as an auxiliary task
    in addition to the standard classification task
  * This setup allows us to incorporate uncertainty about the class memberships
...
* the performance difference between the loss functions is not significant, we
  * nL the inverse version of KL gives the best results in all the experimental
    conditions but one. This finding supports our idea of emphasizing the
* We conjecture that predicting the soft labels acts as a regularizer
