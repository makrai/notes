...
* We propose a comparative approach, nearest neighbor overlap (N2O), that
  ...
  * [count the] overlap between the inputs' nearest neighbors.  Though
...
* evaluations for sentence embedders
  ...
  * Our approach, nearest neighbor overlap (N2O), is something different: it
    * linguisticsand task-agnostic manner, using only a large unannotated
    * By drawing a random sample of queries from the corpus itself, we can
* applications such as 
  * text clustering (Cutting+ 1992), 
  * information retrieval (Salton and Buckley, 1988), and 
  * open-domain question answering (Seo+ 2018), among others.  After 
...
# 3--4 apply it to 21 sentence embedders (§3-4).  Our 
...
* inverse document frequency (how rare the term is across all documents). The

## 3.2 Word Embeddings

### Static embeddings. We define “static embeddings” to be fixed
...
### Contextual embeddings. Contextual word embeddings, where a word token’s
...
### Composition of word embeddings. The simplest way to obtain a sentence’s

* average the word embeddings. 3 Despite the fact that averaging discards word
...
* direct way to obtain sentence embeddings is to learn an encoding function
  * supervised task.  We consider two encoder-based methods:
...
## Averages of static word embeddings. We first

* subword information ... has a large effect on N2O; there is a high (0.52) N2O

## BERT and GPT. We first find that 

* specific-token representations for BERT or GPT are outliers compared to other
  * as the training objectives for both (without finetuning) are not geared
    towards semantic similarity the way other embedders may be.  What is
  * surprising is that this effect seems to hold even for the
    MultiNLI-finetuned version of BERT (bert-ft-cls)
    * this decreases N2O with other embedders further. 7 To further confirm 
    * In preliminary experiments, we also saw similar results with BERT
      finetuned on the Microsoft Research Paraphrase Corpus (Dolan+ 2004); that
* averaged BERT and GPT embeddings yields higher N2O with other embedders,
  especially ELMo-based ones

## Encoder-based embedders. We find that 
...
## Varying k. One possible concern is how sensitive our procedure is to k (

* changing [the number of nearest neighbors] does not significantly affect

## Query sampling. We also examine how the results may vary across different

## Runtime. A theoretical concern with N2O is that, naively, its computation is
...
* “needle-in-a-haystack” experiment 
  * using the Semantic Textual Similarity Benchmark (STS; Cer+ 2017). STS
* We take near paraphrases with moderately different surface semantics. We also
* The first sentence in each sentence pair is the “query,” and the 
* second sentence is (temporarily) added to our Gigaword corpus. An example
* results
