...
* A sclerosis multiplex (SM) a központi idegrendszer krónikus gyulladásos
  * az automatikus beszédföldolgozás relatíve egyszerű, olcsó és távoli módot
...
* tanítóanyag mennyiségének és (főleg) változatosságának korlátozottsága (pl
  ...
  * valamilyen hagyományosabb módszert (pl. SVM-et vagy véletlen erdőt)
    * mély hálók beágyazásait használva jellemzőkként
* mély jellemzők:  Egy korszerű mély háló (pl. wav2vec 2.0) esetén
  kézenfekvő a konvolúciós és a finomhangolt blokkok utolsó rétegét választani,
  * nL közbülső rétegek is hasznosnak bizonyulhatnak. Jelen tanulmányban azt
...
* SM a központi idegrendszer krónikus gyulladásos megbetegedése, mely
  ...
  * kognitív és nyelvi funkciói is (akár dizartria is kialakulhat (Orsolya+ 20)
  [a bib tételben a vezeték és keresztnevek fölcserélődtek]
...
* automatikus beszédelemzés
  * Az elmúlt bő egy évtizedben a beszédtechnológia egy önálló területévé
...
* korlátozottabb mennyiségű tanítóadatnál sajnos nem triviális a betanításuk
  * nL wav2vec 2.0 hálók alkalmazására, 
    * például valamely más feladaton (pl beszédfelismerés vagy beszélőID)
      tanított hálók kiértékelésével és 
      a kapott aktivációk (beágyazások (embeddings)) jellemzőkként
...
* (akkor még) ELKH Nyelvtudományi Kutatóközpontjában került sor
...
* A felvételi protokoll során összesen 17 feladatot rögzítettünk az
  * Jelen tanulmányunkban kísérleteinket a szövegösszefoglalás feladat
    hangfelvételein végeztük; ennek során az alanyoknak egy 
  * kétperces, számukra korábban ismeretlen tudományos ismeretterjesztő szöveg
    meghallgatása után minél pontosabban el kellett azt mesélniük. A feladat
  * számos kognitív funkció  összehangolt működését igényli (Mar, 2004)
  * figyelemösszpontosítás, munkamemória, időbeli orientáció, rendszerezés
...
* A wav2vec [1.0] architektúra egy konvolúciós neurális háló, melynek 
  * bemenete a nyers beszédhang-jel, 
  * kimenete pedig egy olyan reprezentáció, mely alkalmas egy beszédfelismerő
    rendszer bemenetének. 
  * Tanítása önfelügyelt módon történik, az adott felvétel
    következő szegmensének predikciójára (Schneider+ 2019)
* A wav2vec 2.0: zajtűrésének javítása érdekében –
  * maszkolást (a bemenet egyes részeinek nulla értékkel helyettesítését) is
  * kontrasztív tanítás (contrastive learning) alkalmazása, melynek során 
    * a modell célja annak felismerése is, hogy két különbözőféleképpen
      transzformált reprezentáció azonos bemenethez tartozik-e (Baevski+ 2020).
    * két transzformáció esetünkben a finomhangolt réteg kimenete, valamint a
      konvolúciós réteg kimenetének diszkretizált (kvantált) formája
  * architektúra az 1. ábrán
...
* a finomhangolt blokk belső rétegeire koncentrálunk
* Egy standard XLSR-53 hálóban (lásd (Babu+ 2022)) 24 ilyen transzformer réteg
  * kiegészülve a konvolúciós blokk utolsó rejtett rétegével, 25 lehetőséget
* egy mély háló
...
* beszélő-azonosító: A másik fölhasznált modellünk saját tanítású volt,
  ...
  * a (magyar nyelvű) BEA Spontánbeszéd-adatbázis egy részhalmazán
    ...
    felvételvezető hangja is hallható, így összesen körülbelül 
  * 60 órányi beszédanyagot használtunk. Az eredeti sztereó, 44,1 kHz-en
    mintavételezett bemondásokat monó, 16 kHz-es formátumra konvertáltuk. Az
    utolsó rejtett réteg aktivációit kiátlagoltuk (average pooling), az ezt
    követő kimeneti rétegben pedig szoftmax aktivációt használtunk 165
    neuronnal. A betanult modell 
  * beszélőosztályozási hibája a teszthalmazon (de értelemszerűen ugyanezen
    beszélőkön) 1,92% volt. A
  * továbbiakban erre a modellre mint beszélő-azonosító modellre 
* A kinyert beágyazások mindkét háló és minden réteg esetén keretszintűek
  ...
  * jelen munkánkban elsősorban a finomhangolt blokkból nyert beágyazásokra
    fókuszáltunk, 
    ~> a legtöbb teszt során 2048 felvételszintű jellemzőt használtunk; ez
    * kivétel: a konvolúciós blokk utolsó rétegét vizsgáló tesztjeink:
      1024 jellemzőnk volt
* a két wav2vec 2.0 hálóval kapott eredményeket nem lehet közvetlenül összehas,
  hiszen azok tanítása más-más adaton (és vélhetőleg más hiperparaméterekkel)
...
* orvosi beszédfeldolgozási területen egy beszélő felel meg egy ML példának,
  ...
  * keresztvalidációt alkalmaztunk
  * Minden csoportba (fold ba) egy SM és egy kontroll alany felvételei kerültek,
    így összesen 23 csoportot kaptunk. Az osztályozás minőségét a 
  * ROC görbe alatti terület (AUC) metrikával mértük, melynek használata szintén
    igen elterjedt a területen (Carvajal-Castaño+ 2022; Gosztolya+ 2022)
* Osztályozó: SVM-et használtunk (LibSVM implementáció, Chang és Lin, 2011)). A
  * nu-SVM változatot használtuk lineáris kernellel
  * a C hiper- paraméter értékét 10 {−5,...,1} között változtattuk. 
  * Beágyazott keresztvalidációt alkalmaztunk (Cawley és Talbot, 2010):
  * a C hiperparamétert minden tanítás esetén egy további (belső, 22-szeres)
    keresztvalidációs lépés segítségével választottuk ki,
    a ROC görbe alatti terület (AUC) érték alapján
* Az AUC értékek robosztusságának ellenőrzése érdekében
  minden osztályozási kísérletet ötször végeztünk el, melyek
  a 23 beszélőcsoport (véletlenszerű) kialakításában tértek el egymástól. Az
  * ábrákon és a táblázatokban az öt így kapott AUC érték átlagát adjuk meg, míg
  * a modellek robosztusságának vizsgálatához az AUC értékek szórását, valamint
    az értékek terjedelmét ([min; max]) tüntetjük föl
