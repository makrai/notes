...
* Fluent speakers bring an enormous amount of knowledge of comprehension & prod
  * vocabulary: rich representations associated with the words we know,
    ...
    * a useful lens to explore the acquisition of knowledge from text, by both
      people and machines
  * size of adult vocabularies vary widely both within and across languages
  * young adult speakers of American English range from 30,000 to 100,000
    depending on the
    resources used to make the estimate and the
    definition of what it means to know a word
  ...
  * This active vocabulary is extremely limited compared to the size of adult
    * 2000 words for young speakers) and is quite stable, with very few
      additional words learned via casual conversation beyond this early stage
    * a very large number of words to be acquired by some other means
    * children have to learn about 7 to 10 words a day, every single day
  ...
  * The most likely explanation is that
    ...
    * Research into the average amount of time children spend reading, and the
      lexical diversity of the texts they read, indicate that
      it is possible to achieve the desired rate
...
* In this chapter, we expand on static word embeddings in two large ways
  ...
  * pretraining and fine-tuning
    ...
    * The pretrain-finetune paradigm is an instance of what is called
      transfer learning in machine learning: the method of
...
* underlies BERT and its descendants eg RoBERTa (Liu+ 19), SpanBERT (Joshi+ 20)
* In Chapter 9 we explored causal (left-to-right) transformers that
  can serve as the basis for powerful language models LMs that
...
* Bidirectional encoders overcome this limitation by allowing the
  ...
  * contextualized representations of the tokens in an input sequence
    * generally useful across a range of downstream applications
  * self-attention to map sequences of input embeddings (x 1, ..., x n) to
    sequences of output embeddings of the same length (y 1, ..., y n), where
    the output vectors have been contextualized using information from the
...
* the use of the same self-attention mechanism used in causal models
  ...
  * The output vector `y_i` corresponding to each input element `x_i` is
    a weighted sum of all the input value vectors v, as follows:
    `y_i = \sum_{j=i}^n α_{ij} v_j` (11.2)
  * The α weights are computed via a softmax over the comparison scores between
    every element of an input sequence considered as a query and
    every other element as a key,
    where the comparison scores are computed using dot products
    α_{ij} = exp(score_{ij}) / sum_k exp(score ik)
    score_{ij} = q i · k_j
  * Since each output vector, `y_i`, is computed independently,
    the processing of an entire sequence can be parallelized via matrix ops
    ...
    * We then multiply X by the key, query, and value weight matrices (d × d) to
      produce matrices Q ∈ R N×d, K ∈ R N×d, and V ∈ R N×d, containing
      Q = X W^Q; K = X W^K; V = X W^V (11.5)
      => we can compute all the requisite query-key comparisons simultaneously
      by multiplying Q and K^T in a single operation
  ...
  * Fig. 11.3, the full set of self-attention scores represented by QK^T
    ...
    * In the case of causal language models in Chapter 9,
      we masked the upper triangular portion of this matrix (in Fig. ??) to
      eliminate information about future words
      since this would make the language modeling training task trivial
    * With bidirectional encoders we simply skip the mask, allowing the model
...
* The result is a model with over 100M parameters
  * WordPiece (one of the large family of subword tokenization algorithms that
    includes the BPE algorithm we saw in Chapter 2)
  * based on subword tokens rather than words
    * Every input sentence first has to be tokenized, and then
      all further processing takes place on subword tokens rather than words
    * for some NLP tasks that require notions of words (like NER, or parsing)
      we will occasionally need to map subwords back to words
* issue with transformers is that
  ...
  * Both the time and memory requirements in a transformer grow quadratically
    ...
    => necessary to set a fixed input length that is
    long enough to provide sufficient context for the model to function and yet
    still be computationally tractable
...
* original approach to training bidirectional encoders is called Masked Language
  ...
  * a random sample of tokens from each training sequence is selected for learn
...
* With a predicted probability distribution for each masked item, we can use
  cross-entropy to compute the loss for each masked item
...
* For many NLP applications, the natural unit of interest is larger than a word
  * eg Question answering, syntactic parsing, coreference and semantic role labl
  * identification and classification of constituents, or phrases
  * a span-oriented masked learning objective might improve performance
* A span is a contiguous sequence of 1 or more words before subword tokenization
  ...
  * SpanBERT originated this technique (Joshi+ 2020),
    * a span length is first chosen by sampling from a geometric distribution
      * biased towards shorter spans and with upper bound of 10
    * a starting location is sampled uniformly
      * consistently with the desired span length and the length of the input
...
* Downstream span-based applications rely on
  ...
  * Representations for these boundaries are typically derived from
    the first and last words of a span,
    the words immediately preceding and following the span,
    or some combination of them
  * The SpanBERT learning objective augments the MLM objective with
    a boundary oriented component called the Span Boundary Objective (SBO)
    * SBO: predict the words within a masked span
      from the words immediately preceding and following it
      * prediction is made using
        * the output vectors associated with the words that immediately precede
          and follow the span being masked, along with
        * positional embedding that signals which word in the span is being pred
    L(x) = L_MLM (x) + L_SBO (x) (11.7)
    L_SBO (x) = −log P(x|x s, x e, p x) (11.8)
    * where s denotes the position of the word before the span and
      e denotes the word after the end
    * The prediction for a given position i within the span is produced by
      ...
      s = FFNN([y s−1; y e+1; p i−s+1 ])
      z = softmax(E s)

### 11.2.3 Next Sentence Prediction

* masked-based learning (pred words from surrounding contexts) is for word repr
...
* BERT introduced a second learning objective called Next Sentence Prediction
  ...
  * predict whether each pair consists of an actual pair of adjacent sentences
  * 50% of the training pairs consisted of positive pairs, and in the other 50%
    the second sentence of a pair was randomly selected
  * two new tokens to the input representation (tokens that
    ...
    * [CLS] is prepended to the input sentence pair, and the token [SEP] is
      placed between the sentences and after the final token of the second sent
...
* During training, the output vector from the final layer associated with the
  ...
  * a learned set of classification weights W NSP ∈ R 2×d h is used to produce a
    two-class prediction from the raw [CLS] vector.  `y_i = softmax(W NSP h i)`
...
* Given a pretrained language model and a novel input sentence
  * the output of the model = contextual embeddings for each token
...
* Contextual embeddings are thus vectors representing some aspect of the meaning
  * we can use the output vector `y_i` from the final layer of the model as a
    representation of the meaning of token `x_i` in the context of sent OR
  * it’s common to average the output tokens `y_i` from last four layers of the
* Where static embeddings represent the meaning of word types (vocabulary entry)
  ...
  * In the next sec: their most common use:
    as embeddings of word or even entire sentences that are the inputs to
    classifiers in the fine-tuning process for downstream NLP applications

## 11.3 Transfer Learning through Fine-Tuning 11

* pretrained language models can extract generalizations from large text
* useful for myriad downstream applications
...
* Seq classif applics often represent an input sequence with a single repr
...
* eg sentiment classification
  ...
  * Classification of unseen documents proceeds by passing the input text
    ...
    W C, and finally passing the resulting vector through a softmax
...
* Fine-tuning an application for one of these tasks proceeds just as with
  ...
  * classification, the [CLS] vector is multiplied by a set of classific weights
    and passed through a softmax to generate label predictions
* eg entailment classification task with the Multi-Genre NLI (MultiNLI) dataset
  (Williams+ 2018)
  * natural language inference or NLI, also called recognizing textual entailm
  * classify the relationship between the meanings of a pair of sentences
  * 3 labels: entails, contradicts and neutral
    * The meaning of these labels is looser than strict logical entailm/contradi
      * a typical human reading of the sentences would most likely be this way
...
* To fine-tune a classifier for the MultiNLI task, we pass the
  ...
  * use the output vector for the [CLS] token as the input to the
    __classification head__
  * this head provides the input to a three-way classifier that can be trained

### 11.3.3 Sequence Labelling

* eg part-of-speech tagging or BIO-based named entity recognition
  * the same basic classification approach
  * the final output vector corresponding to each input token is passed to a
    classifier that produces a softmax distribution over the set of tags
  * simple classifier consisting of a single feedforward layer + softmax, the
    ...
    * k is the number of possible tags for the task
    `y_i = softmax(W^K z i)` (11.12)
    `t i = argmax k (y_i)` (11.13)
  * tag seq
    * As with RNNs, a greedy approach can be used
      * ie the argmax tag for each token is taken as a likely answer
...
* complication with this approach arises from the use of subword tokenization
  * Supervised training data for tasks like named entity recognition (NER) is
    typically in the form of BIO tags associated at the word level
  * `t_i` doesn’t align directly with BIO tags in the ground truth annotation
...
* middle ground between sequence level and token level tasks
...
* start i and end j such that 1 <= i <= j <= T
  * often impose an application-specific length limit L
  * refer to the enumerated set of legal spans in x as S(x)
* fine-tuning a pretrained language model for a span-based application using
...
* simplest possible approach, we can use the contextual embeddings of the start
  and end tokens of a span as the boundaries, and
  the average of the output embeddings within the span as the summary repr
* more elaborate
  * boundaries: distinguish the embedding of a word as the beginning vs end
    * two distinct feedforward networks:
    s i = FFNN_start (h i) (11.17)
    e j = FFNN_end (h j)   (11.18)
...
* eg named entity recognition (NER)
  ...
  * During decoding, each span is scored using a
    softmax over the final classifier output
...
* Large pretrained neural language models exhibit many of the potential harms
  ...
  * eg generate toxic language
    ...
    * Brown+ (2020) and Sheng+ (2019); large language models generate sents with
      negative attitudes toward minority identities such as Black or gay
* biased in a number of ways by the distributions of their training data
  * Gehman+ (2020) shows that large language model
    training datasets include toxic text scraped from banned sites
  * internet data is disproportionately generated by authors from developed
    ...
    * male and young. Such biased population samples likely skew the model
...
* privacy issues. Language models, like other machine learning models,
  ...
  * possible for an adversary to extract individual training-data phrases
...
* Mitigating all these harms is an important but unsolved research question
  * Extra pretraining (Gururangan+ 2020) on non-toxic subcorpora reduces a
