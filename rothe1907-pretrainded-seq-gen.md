...
* extensive empirical study on the utility of 
...
* argued that the pre-training objective used by BERT is not well suited for
  ...
  machine translation and summarization (Yang+ 2019).  Nevertheless, it 
...
* what is the best way to leverage publicly available pre-trained checkpoints
  ...
  * we rigorously experiment with a large number of different settings to
    ...
    * sentencelevel fusion (DiscoFuse, Geva+ 2019) and 
    * splitting (WikiSplit, Botha+ 2018)), 
    * WMT14 En↔De machine translation using most common eval sets: newstest2014
      and newstest2016, and 
...
* significant improvements over randomly initialized models demonstrating the
* new SOTA results
...
* If not stated otherwise, the implementation of the 
  ...
  * we add an encoder-decoder attention mechanism.  Note, that if the model was

# 3 Investigated Model Variants

* GPT A decoder-only architecture. 
  * We treat the input as a conditioning prefix of a language model. The decoder
...
* BERT S HARE and ROBERTA S HARE summaries are unequivocally better than RND 2
  GPT summaries in terms of both automatic evaluations (assessing ROUGE) and
  human evaluations (assessing summary quality) 
* truncation
...
* The BERT S HARE setup with shared encoder and decoder parameters achieves
  better performance than BERT 2 BERT on all three datasets. The 
...
* ROBER TA S HARE performs ?> to BERT S HARE 
  * superior on the CNN/DailyMail and BBC datasets. ROBER TA S HARE performs
  * competitively on the Gigaword dataset where the input is sentences

### Summarization with GPT checkpoints. GPT

* GPT (decoder-only) ?> than RND 2 GPT , BERT 2 GPT or ROBERTA 2 GPT (enc-dec)
  * better by a large margin for generating CNN/DailyMail extracts, but 
  * poorer for generating BBC abstracts. The 
  * encoder-decoder architecture where the input document is modeled separately
    is better equipped for document-level abstraction than the decoder-only
    architectures where the input document is a conditioning prefix of a
    language model. 
* Initialization with different checkpoints, e.g. BERT 2 GPT
* Gigaword: ROBERTA S HARE performs the best and is 
  * on par with the current SOTA MASS model (Song+ 2019) on the 
* BBC extreme: The ROBERTA S HARE setup sets a new SOTA, outperforming all
  existing baselines by a large margin on the summarization task. 
...
* Combining BERT and GPT-2 into a single model ( BERT 2 GPT ) did not work and
* presumable because the model has to learn two different vocabularies. This
  * backed by the fact that for MT de→en the BERT 2 GPT setup performed well.
    * two vocabularies have to be learned anyways and 
    * the output is English, where GPT-2 was trained on. Since RoBERTa and GPT-2
...
* sometimes the vocabulary given from a public checkpoint might not be optimal
  for a certain task. In these cases, it would be nice to redefine the
...
* a more sensitive way of training the model, meaning that 
  * for 100k steps, we only train the new word embeddings. After the
  * then we fine-tune the entire model for another 300k steps. This training
  * still way behind the fully initialized setup. For GPT , this training scheme

## Initializing a Subset of Layers. Motivated by

* still has more parameters then the base model, but can be trained with the
  same batch size, in a comparable amount of time (3 min/1000 iterations). As an
* hyperparameter search on the evaluation set using CMA-ES (Hansen, 2016) to
  ...
  layers: 9, 10, 13-18, 23, 24; and achieved a SARI score of 89.1. While this is
* still outperformed by the base BERT model.

# 6 Analysis of Abstractive Summaries 9

## Human Assessment of Summary Quality. The

* Our participants found the ROBERTA S HARE summaries to be the best in terms of
  their overall quality; the BERT S HARE summaries ranked second 
* whether system differences are statistically significant

## Summary Lengths and Repetitions. 

* All models generated summaries of comparable lengths; the average length of
  * ROBERTA S HARE produced summaries were closest to the G OLD summaries in
