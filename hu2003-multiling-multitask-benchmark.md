...
Junjie Hu, Sebastian Ruder, A Siddhant, Graham Neubig, Orhan Firat, Melvin Johnson
...
* 40 languages and 9 tasks.  We demonstrate that while 
  * models tested on English reach human performance on many tasks, there is
...
* a pressing urgency to ... serve all of the world’s approximately 6,900 langs
  * overcome language barriers and enable universal information access for the
    world’s citizens (Ruder+ 2019; Aharoni+ 2019; Arivazhagan+ 2019b). At the
...
* general-purpose multilingual representations that are applicable to many
  * word level (Mikolov+ 2013; Faruqui & Dyer, 2014; Artetxe+ 2017) or the
  * full-sentence level (Devlin+ 2019; Lample & Conneau, 2019). Despite the
  * evaluation [ often ] on a very 
    * limited and disparate set of tasks—typically
...
* XTREME covers 
  ...
  * focuses on the zero-shot cross-lingual transfer scenario, where 
    * annotated training data is provided in English but none is provided in tg
      the language to which systems must transfer. 
    * both for 
...
* SOTA models come close to human performance in English on many of the tasks
  * performance drops significantly when evaluated on other languages. Overall,
  * differences are highest for syntactic and sentence retrieval tasks.
  * reasonably [good] in most languages in the Indo-European family, we observe
...
* contributions are the following: 
  ...
  * strong baselines, which we evaluate across all tasks, and release code to
...
* using either parallel corpora (Gouws+ 2015; Luong+ 2015) or a 
  ...
  (Mikolov+ 2013; Faruqui & Dyer, 2014). Later approaches reduced the amount of
...
* unsupervised strategies such as 
  ...
  * optimal transport (Zhang+ 2017). Building on advances in 
...
* translation, either on the 
  ...
  * sentence level (machine translation). In most cases, evaluation has been
  ...
  * Translation ... not capture differences in classification performance that
    are due to cultural differences (Mohammad+ 2016; Smith+ 2016)
...
* Evaluation on a single task is problematic: issues with standard datasets:
  MLDoc (Schwenk & Li, 2018) can be solved by matching keywords (Artetxe+ 2019), while 
...
### Task difficulty Tasks should be sufficiently challenging i.e.e `<` human

### Task diversity

* transfer ... at different levels, e.g. words, phrases and sentences.  For
...
* Tasks should be trainable on a single GPU for less than a day. This is to
...
### Accessibility: tasks should be available under a permissive license
...
* four different categories requiring reasoning on different levels of mean-
...
* eval: Cross-lingual Natural Language Inference corpus (Conneau+ 2018b) 
  * whether a premise sentence entails, contradicts, or is neutral toward a hypo
...
* determine whether two sentences are paraphrases. A subset of the PAWS dev and
* translated to six other languages by professional translators and is used for

### POS from the Universal Dependencies v2.5 (Nivre+ 2018) treebanks, which

* 90 languages. Each word is assigned one of 17 universal POS tags. We use the
...
* answer to a question as a span in the corresponding paragraph. A 
...
### MLQA The Multilingual Question Answering (Lewis+ 2019) dataset is

* similar to XQuAD. The evaluation data for English and six other languages was

### TyDiQA-GoldP 

* We use the gold passage version of the 
  Typologically Diverse Question Answering (Clark+ 2020) dataset, a benchmark
  * information-seeking question answering, which covers nine languages. The
  ...
  * similar to XQuAD and MLQA, while being more challenging 
    ...
    3× and 2× less lexical overlap compared to XQuAD and MLQA respectively. We
...
### Tatoeba dataset (Artetxe & Schwenk, 2019), which consists of up to 

* 1,000 English-aligned sentence pairs covering 122 languages.  We find the
  nearest neighbour using cosine similarity and calculate error rate.
