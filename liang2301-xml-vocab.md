...
* Large multilingual LMs: typically a single vocabulary for 100+ languages
...
* we introduce a new approach for scaling to very large multiling vocabularies
  ...
  * our Tokenizations are typically more semantical and shorter than XLM-R
    * reduces average sequence length
* we train XLM-V, a multilingual language model with a one million token vocab
  * outperforms XLM-R on every task we tested on
    * natural language inference (XNLI), question answering (MLQA, XQuAD,
      TyDiQA), to named entity recognition (WikiAnn)
...
* For models like mT5 and XLM-R, this 250K vocabulary is shared across 100+
  * Discounting shared tokens, this results in an average of
    2,500 unique tokens per language, calling into question the vocabulary
* eg Chinese
  * 8,105 characters in the Table of General Standard Chinese
  * over 100,000 unique characters in total
  * commonly used Chinese words (consisting of multiple characters) is more
...
* we construct a large multilingual vocabulary with 2 principles
  * vocabularies can be improved by de-emphasizing token sharing between
    languages with little lexical overlap and
...
* favorable characteristics including the ability to
  ...
  * reducing over-tokenization for lowresource languages
...
* Chung+ (2020) proposed an approach to multi-lingual vocabulary construction
  * balances the trade-off between optimizing for cross-ling sub-word sharing
    and the need for robust representation of individual languages
* steps
  ...
  * Next, for each language l, they construct a binary vector v l of dimension
    ...
    corresponding to each subword present in the vocabulary of l, see fig 1
...
* Zheng+ (2021) proposed the average log probability (ALP) to evaluate the
  ability of a vocabulary to represent a particular language
  * skj is the k-th subword of the sentence sj and
    `p_uni (·)` is the unigram distribution counted on the monolingual corpus
* ALP is highly correlated with downstream task performance
* greedy algorithm to determine the desired vocabulary capacity for individual
  languages in the multilingual vocabulary

# 3 Method for constructing large multilingual vocabularies
...
* Unlike Chung+ (2020), where a language is represented by a binary vector
  containing a 1 corresponding to each subword present in the vocabulary
* we instead use the negative log probability
  that each token appears in the respective language’s monolingual corpus
...
* fig 3 visualizes the diminishing utility of increasing the vocabulary size
  * vocabularies with 500K, 1M, 1.5M, and 2M tokens using our methodology
  * tokenize the FLoRes-200 dataset
...
* We hypothesize that since the Unigram LM (Kudo and Richardson, 2018)
  ...
  * These token embeddings are trained on significantly less data
...
* For languages in the same cluster (en, fr, es), our method can separate
  shared roots (eg narco) from the same word in different languages
...
* Table 6 shows the average number of resulting tokens (post-tokenization) for
  several languages in FLoRes-200
...
* scaling the vocabulary can markedly increase the number of trainable params
  * nL an efficient form of conditional compute (Bengio+ 2015):
    only a small fraction of the embedding matrix is used for any given input
* fig 4: the relationship between the vocabulary size and training speed
...
* 1M token vocabulary. We showed that our model outperforms XLM-R
* future
...
* it can result in increased pre-training times due to the computational
  complexity of the softmax over the entire vocabulary
  * can be solved by adopting approximation techniques like adaptive softmax
    (Joulin+ 2017) and adaptive inputs (Baevski and Auli, 2018)
* it can also significantly increase the memory footprint of a model
  * nL less of a problem with larger models, where the number of non-embedding
