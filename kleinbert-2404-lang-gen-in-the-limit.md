...
* given a finite set of training samples from an unknown language, 
  produce valid new strings from the language that don't already appear
...
* suppose that an adversary enumerates the strings of an unknown target language
  * a possibly infinite list of candidates
* we say that the agent generates from L in the limit 
  ...
  that have not yet been presented by the adversary
* we: there is an agent that is able to generate in the limit 
  for every countable list of candidate languages
* This contrasts dramatically with negative results due to Gold and Angluin
  * Gold and Angluin: well-studied model of language learning where 
    the goal is to identify an unknown language from samples
