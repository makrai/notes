...
* Lee+ (2020): a new regularization method to constrain the fine-tuned model
  to stay close to the pre-trained weights.  It stabilizes
* Dodge+ (2020): an early stopping method to efficiently filter out rand seeds
  likely to lead to bad performance
...
* The most commonly used optimizer for fine-tuning BERT is BertAdam ,
  ...
  * differs from the original Adam algorithm (Kingma & Ba, 2014) in omitting
    a bias correction step. This change was
  * introduced by Devlin+ (2019), and subsequently made its way into
  * in common open source libraries, including the official implementation, 5
    huggingface’s Transformers (Wolf+ 2019), 6 AllenNLP (Gardner+ 2018),
    GluonNLP (Guo+ 2019), jiant (Wang+ 2019c), MT-DNN (Liu+ 2020), and FARM. 7
...
* Algorithm 1 shows the Adam algorithm, and
  ...
  * At each optimization step (lines 4–11), Adam computes the exponential
    moving average of the gradients (m t ) and the squared gradients (v t ),
    where β 1 , β 2 parameterize the averaging (lines 7–8)
  * Because Adam initializes m t and v t to 0 and sets exponential decay rates
    β 1 and β 2 close to 1, the estimates of m t and v t are heavily biased
    towards 0 early during learning when t is small
  * Kingma & Ba (2014) computes the ratio between the biased and the unbiased
    estimates of m t and v t as (1 − β 1 t) and (1 − β 2 t )
    * This ratio is independent of the training data
  * The model parameters θ are updated in the direction of the averaged
    gradient m t divided by the square √ root of the second moment v t (l 11)
  * BertAdam omits the debiasing (lines 9–10), and directly uses the biased
    estimates in the parameters update
* Figure 1 shows the ratio √ m̂ v̂ t between the update using the biased and the
  ...
  * => the bias ratio term is most important early during learning to
    counteract the overestimation √ of m t and v t during early iterations
  * In practice, 1−β t Adam adaptively re-scales the learning rate by 1−β t 2
  * correction is crucial for BERT fine- tuning on small datasets with fewer
    than 10k training samples because they are typically fine-tuned with less
    than 1k iterations (Devlin+ 2019)
    * The figure shows the number of training iterations for RTE, MRPC, STS-B,
      CoLA, and MNLI
    * MNLI is the only one of this set with a large number of supervised
      training examples
...
* We evaluate the importance of the debiasing step empirically by fine-tuning
  BERT with both BertAdam and the debiased Adam 9 for 50 random seeds
  on RTE, MRPC, STS-B, and CoLA
  * Figure 2 summarizes the performance distribution
  * The bias correction significantly reduces the performance variance across
    different random trials and the four datasets
  * Without the bias correction we observe many degenerate runs, where
    fine-tuned models fail to outperform the random baseline.  For example, on
    * eg RTE, 48% of fine-tuning runs have an accuracy less than 55%, which is
      close to random guessing
  * Figure 3 further illustrates this difference by plotting the mean and the
    range of training loss during fine-tuning across different random trials on
    RTE
    ...
    * The biased BertAdam consistently leads to worse averaged training loss,
      and on all datasets to higher maximum training loss. This indicates
      models trained with BERTAdam are underfitting and the root of instability
...
* During fine-tuning, the BERT parameters take the role of the
...
* We study the value of transferring all the layers
  vs ignoring the information learned in some layers.  This is
  * motivated by object recognition transfer learning results showing that
    ...
    higher layers closer to the output specialize more to the pre-training
    tasks (Yosinski+ 2014)
...
* BERT is typically fine-tuned with a slanted triangular learning rate, which
  * linear warm-up to the learning rate followed by a linear decay. This
  * warrants deciding the number of training iterations upfront [in advance]
  * Devlin+ (2019) recommend fine-tuning GLUE datasets for three epochs.  This
...
* the debiasing omission in BertAdam is the main cause of degenerate models on
  small datasets commonly observed in previous work (Phang+ 2018; Lee+ 2020;
  Dodge+ 2020)
* the top layers of the pre-trained BERT provide a detrimental initialization
  ...
  * Simply re-initializing these layers not only speeds up learning but also
    leads to better model performance. Third, we
...
* we revisit several methods proposed for stabilizing BERT fine-tuning and
  * their positive effects are reduced with the debiased Adam .  In the
