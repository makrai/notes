...
* important role of structured lexical representations in natural language, for
  * argument structure as a distinct level of representation
    (Grimshaw, 1990; Williams, 1981) and the importance of semantic
...
* arg struct
  ...
  * Some authors: a structured representation over which prominence relations
* events (e.g., whether a sentence denotes an activity, a state, etc.)
  ...
  * There are many who still argue that events are not necessary type
    (Cresswell, 1985)
  * we will argue that grammatical phenomena do in fact make reference to the
    internal structure of events
    * subeventual analysis for predicates is able to systematically capture
      these effects
* basic assumptions of the theory are as follows: (I)
  1. A subeventual structure for predicates provides a template for
    verbal decomposition and lexical semantics. Following
    * Vendler (1967), Dowty (1979), and others, we distinguish three event types
...
* at least three aspectual types: state, process, and events,
  ...
  * process eg _walk_: an activity of indefinite length. That
    * no information regarding the temporal extent of the activity. Such a
    * (Dowty, 1979; Kenny, 1963; Mourelatos, 1978; Ryle, 1949; Vendler, 1967;
      Verkuyl, 1989). Sentence (1b) conveys at least as much information
...
* achievement
  * like accomplishment: results in a change of some sort, just as an
...
* What are apparently lexical properties of the verb
  can be affected by factors that could not possibly be lexical. For instance,
  * eg (6) accomplishment as in (6a) to a process as in (6b):
    (6) a,b Mary drank (a) beer.
  * bare plural object shifts the interpretation of a logically culminating
    event to an unbounded process
...
* states, such as those sentences in (7):
  ...
  * diagnostics.
    ...
    * not appear as imperatives (8b) (Dowty, 1979):
      b. `*`Be sick!
* a few more diagnostics. One well-known test for distinguishing
  ...
  * accomplishments vs non-accomplishments based on scalar adverbs eg _almost_
    ...
    b allows for two interpretations. One is similar to (10a), where John
      * other interpretation is that John was painting but did not complete the
...
* If aspectual type is viewed as a projection of the lexical properties of the V
  ...
  * we will explain them in terms of a configurational theory of event struct
    * how the tests normally used as diagnostics for asp type fall out
      as the principled consequence of different event structures. Thus, we are
    * abandon the feature-based approach to aspect which is generally assumed
      (e.g., Verkuyl, 1972)
      in favor of a restricted, topological theory of how events are structured

# 3 Towards a model of lexical knowledge 5

* the basic requirements for a theory of lexical semantics should be. We present
  * a conservative approach to decomposition, where
    lexical items are minimally decomposed into structured forms (or templates)
    rather than sets of features. We will provide a
...
* relation-based theory of word meaning claims that there is
  * no need for the decomposition of word meaning into primitives if
  * words are associated through a network of explicitly defined links
    (Brachman 1979; Carnap 1956; Collins & Quillian 1969; Fodor 1975;
     Quillian 1968)
    * link Sometimes referred to as meaning postulates, these links
      * establish any inference between words
        as an explicit part of a network of word concepts. 3 What we would like
...
* approaches to lexical semantics making use of primitives can be characterized
  ...
  * provide a characterization for all of the well-formed expressions in the l?
    * not Even those theories that rely on some internal structure for word mean
      (e.g., Dowty, 1979; Fillmore, 1988) do
    * Jackendoff (1983) comes closest, but falls short of a comprehensive
      semantics for all categories in language. No existing framework, in my
* What, exactly, would a method for lexical decomposition tell us?
  * Instead of a taxonomy of the concepts in a language, categorized by
  * minimal semantic behavior of a lexical item. Furthermore,
  * the compositional properties of a word, just as a grammar informs us of the
...
* Lakoff (1970), Jackendoff (1972), and others have suggested that
  the sense in (12c) must incorporate something like cause-to-become-not-open
  ...
  * decomposition theories of word meaning (with a set of primitives) are
    exhaustive since they assume that
...
* A minimal decomposition of the word closed is that
  ...
  * draws on Aristotle ’s species of opposition
    (see Categories, 11b17; Lloyd, 1968), and it will form the basis of one
  * evaluate words relative to an opposition. 6
...
* The minimal semantic distinctions of the qualia structure
  ...
  * Both the opposition of predicates and the specification of causation are
    ...
    `formal` role for the verb _close_ is specified as a transition, denoting an
    opposition
    `constitutive` role specifies the exact content of the event, in terms of
    what the predicates engaged in opposition actually are (see below)
...
* three basic event types: states, processes, or transitions
  (see Bach, 1986; Dowty, 1979; Vendler, 1967). 9 Except where
...
* Following Dowty (1979) and others, we will assume that
  ...
  P is true at an interval I, then P 0 is true for all subintervals of I larger
  than a moment
...
* assume a level of lexical representation
  similar to that of Dowty (1979), Jackendoff (1983), Levin and Rappaport (1988)
  in that verb class distinctions are characterized in terms of an LCS-like
  structure, which we call LCS 0 .
  * An LCS is a lexical semantic representation
    in the form of a predicate decomposition. Here
  * in terms of
    * no fixed set of primitive terms
    * the principles of event structure outlined here. This level, using
  * predicates such as act(x, y), at(x, y), and on(x, y), constitutes the LCS 0
  * the trees represent LCS-like information
    partitioned according to event structure. Furthermore, we will assume that
* the LCS representations assumed by Levin and Rappaport (1988)
  can be constructed by interpreting the event structure together with the LCS 0
* eg
  * The adjectival form closed denotes a state where the event structure is that
  * a lexical transition, such as close in (12b) and (12c), expresses the
    ...
    * the operator cause can be seen as a derivative relation between events,
      structurally interpreted from an agentive predicate within the initial
      subevent of an event structure
* LCS
  a.  [closed(the-door)]
...
* `LCS 0` representations differ only in the further specification of an
  action being performed for the causative cases; that is, the expression and
...
* Thus, while the verb close is semantically ambiguous at one level (LCS), the
  logical relatedness of the verb ’s senses is captured at another (ES , LCS 0)
...
* The aspectual distinctions made by the above trichotomy do
  not distinguish between achievements and accomplishments in any structural
...
* The role of agentivity for distinguishing event types (Dowty 1979),
  ...
  * differ in their event structures eg
    ...
    arrive and die
* hE, an agentive role does seem implicit in many achievement predicates. For
...
* is agency part of the verbal semantics of arrive and win
  ...
  pragmatic effects.
  * agency is not intrinsically part of the verb ’s meaning is not as
