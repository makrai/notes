...
* questions the need for such a parameter-loaded model, especially on a single
  downstream task
...
* our paper: how many of the last layers actually need fine-tuning
  * Why is this an important subject of study? 
    * saves computational memory across fine-tuning multiple tasks, which
      bolsters the effectiveness of existing parameter-saving methods (Houlsby+
      2019)
    * understanding the relationship between the number of fine-tuned layers
      and the resulting model quality may guide future works in modeling.
...
* future work is similar, more fine-grained analysis on the attention heads
