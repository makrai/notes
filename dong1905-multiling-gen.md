...
* This paper presents a new Unified pre-trained Language Model (UniLM) that 
  ...
  * pre-trained using three types of language modeling tasks: 
    unidirectional, bidirectional, and sequence-to-sequence prediction. The
...
* we propose a new U NI fied pre-trained Language Model (UniLM) that 
...
* three types of unsupervised language modeling objectives as shown in Table 2.
  ...
  * left-to-right unidirectional LM, the context is the words on its left. For a
    right-to-left unidirectional LM, the context is the words on the right. For
  ...
  * sequence-to-sequence LM, the context of a word in the target sequence
    consists of all the words in the source sequence and the words on the its
    left in the target sequence.  
...
* Experimental results show that our model, used 
  * as a bidirectional encoder, compares favorably with BERT 
    on the GLUE benchmark and two extractive QA tasks (SQuAD 2.0 and CoQA). In
