...
* the first theo (2nd in sec 6)
  ...
  * Predicative uses are explained as elliptic attributive uses
    ...
    * Which noun phrase ought to be supplied: not comply determined by the sent
...
* We may call an adjective
  * predicative in a given interpretation if (4) there is a property Q such
    ...
    * Predicative adjectives behave essentially as independent predicates
      ...
      * _If every N1 is an N: then every four-legged N1 a four-legged N_
  ...
  * are there adjectives which are extensional but not predicative
    * suggested; _small, tall, heavy_, and _hot_ belong to this category
    ...
    (8) for any property P and world w
...
* naive point of view: the comparative is an operation which forms out of an
  ...
  * when we learn a language such as English we learn the meanings of indiv adjs
    and the semantic function which this comparative-forming operation performs
  * the meaning of an adjective must be such that
    the comparative can be understood as a semantic transformation of that
    meaning into the right binary relation
...
* natural to require of a many-valued model theory for L
  (10) there are two particular elements of TV —let us call them o and 1 —
    * 1 \in TV_t, absolute truth
    * 0 \notin TV_t, absolute falsehood
    * for each connective C, the restriction of F(C) to {0,1} is the usual
* those model theories for `L_0`, which have been proposed in liter satisfy (10)
...
* two-element sets are the only linearly ordered truth-value sets which can be
  ...
  * If we define the Boolean operations \cap, U, — in the usual manner
    in terms of the ordering relation < of a linearly ordered set TV
    * i.c. if we put
      * C \cap C’ := the largest C” under < such that C"<C and C"<C"
      * C U C’ := the smallest C” such that C<C” and C’ <C"
...
* choose as truth-value sets rather than linear orderings
  ...
  * We may then further ‘reduce’ these Boolean algebras to linearly ordered syss
    * different Boolean values may be assigned the same elem of the linear order
...
* idea is by now quite familiar to logicians
  ...
  * the theory of probability in the definitive form by Kolmogorov (1970)
    * one associates with a proposition in first instance a certain set
    * With this set is assoced in turn a real number in [0:1]
      * the probability of the proposition
    * the set assoced with the conjunction of two propositions is just
      the intersection of the sets assoced with the conjuncts
    * hE, there is no way of telling in general the probability of the
      conjunction on the basis of just the probabilities of the conjuncts; and
* the main philosophical problem which this approach raises is the
  ...
  * I will consider here only one doctrine:
  * the elements of the sets are regarded as possible worlds/situations
  ...
  * In intensional logic we are primarily concerned with the analysis
    in terms of the set of all possible ‘worlds’
    (as well as various structural properties of this set)
    of the semantical function of certain non-truth functional operators
    eg _it is necessarily the case that_
  * probability theory
...
* partial interpretations and supervaluations
  ...
  * in its simplest form, a generalization of ordinary two-valucd model theory,
    which
...
* this process again assigning sets to sentence interprets
  ...
  * for an analysis of vagueness
    ...
    * apply not only to adjectives but other POSs as well, in particular to
      ...
      * later: systematic differences between the sem of adjs and these two
...
* A partial model for L is a pair (U,F) where
  ...
  (ii) F assigns to each letter `Q_i^n`
    an ordered pair (F^+(Q_i^n),F^-(Q_i^n)) of disjoint n-place-relations on U
* The partial model M = (U,F) is said to be at least as vague as the partial
  model M’ = (U,F’) (in symbols: M \subseteq M’)
* To each classical model (U,F) for L corresponds a unique partial model
  * ie partial models are generalizations of classical models

# 4 supervaluation, degree of satisfaction, degree of truth

* supervaluation
  * one considers partial models in conjunction with __all__ their completions
  ...
  * a vague model: The complex consisting of
    ...
    the probability function over that field
    * Formally A vague model for L is a quadruple
      ...
      * for each \phi \in L and assignment `a` in the universe of M,
        {M' \in C: [\phi]_{M',a} = 1} \in F
      * p is a probability measure on F
* Let M  = ((U,F),C,F,p) be a vague model for L. For any formula \phi of L and
  assignment `a` of elements of U to the variables
  __the degree of satisfaction__ of \phi by a in M, `[\phi]_{M,a}`, is defined
  as `p({M’ \in C: [\phi]_{M',a} = 1})`
* The idea behind the notion of a vague model is this: language is vague
  * eg the adjective _intelligent_
...
* My original motivation in setting up this framework was to give
  a uniform characterization of the operation
  which transforms adjectives into their comparatives
  * The relation `x is more A than y` (where A is any adjective) can be defined
    in terms of the relation `x is at least as A as y`
...
* positive extension in M (neg ext in sec 5, page 145)
  * Two possible definitions come to mind
  12. an element u_1 of U stands (definitely) in the relation to u_2,
    if for every member M’ of C in which u_2 belongs to the extension of Q
    u_1 belongs to that extension as well
  13. u_1 stands in the relation to u_2 if
    the measure of the set of completions in which u_1 belongs to the extension
    of Q is at least as large
    as that of the set of completions in which u_2 belongs to the extension
  * a flaw which the 2 defs share
    ...
    * For if both u, and 2, belong to F^+(Q!) then [Q(v_1)]_{M,a1} =
      [Q(v_1)]_{M,a2} = 1
* we need to consider models M in which the set C comprises
  besides completions of M
  also complete models which in certain ways conflict with M
...
* eg
  ...
  * What should in this case C and p be?
    * C: For each real number r there will be a member M of C in which the
      extension of Q consists of those objects whose weight (in grams) exceeds r
    * It is not possible to say precisely what the function p should be
    * there should be a strictly monotonic function f
      from the set of all positive real numbers into the interval [0,1]
      so that for any object u with weight r, p([Q(v)]_{M,a} = f(r)
      (for some a with a(v) = u)
...
* A graded context-dependent model (cgm) for L is a quintuple (M, S,6,F P) where
  ...
  * (a) for each formula ¢ and assignment a the set of members AZ’ of FY such
...
(19) with regard to shape w, is more a table than u,?
  ...
  * Then (19) should be true in the ground model if and only if «, 7s more a
    table than u, is true in each of these contexts
