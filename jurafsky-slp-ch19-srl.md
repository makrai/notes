...
* Sometime between the 7th and 4th centuries BCE, the Indian grammarian Pānini
  wrote a famous treatise on Sanskrit grammar, the Astādhyāyı̄ (‘8 books’)
  * in the form of 3959 sutras, each very efficiently (since it had to be
    ...
    brilliantly prefigured modern mechanisms of formal language theory
    (Penn and Kiparsky, 2012)
...
* eg task of understanding text about a purchase of stock by XYZ Corporation
  * event and participants can be described by a wide variety of surface forms
    * event can be described by a verb (sold, bought) or a noun (purchase)
    * XYZ Corp can be the syntactic subject (of bought), the indirect object (of
      sold), or in a genitive or noun compound relation (with _purchase_)
* In this chapter we introduce
  ...
  * shallow semantic representations, semantic roles
  * repr the role that arguments of a predicate take in the event
  * databases like PropBank and FrameNet
...
∃e, x, y Opening(e) ∧ Opener(e, Pat) ∧ OpenedThing(e, y) ∧ Door(y)
  ...
  * for sem processing
    eg answer questions, perform inferences, or do any further kinds of sem
...
* Thematic roles capture this semantic commonality between Breakers and Openers
  * the subjects of both these verbs are agents
    * AGENT represents an abstract idea such as volitional causation
  * the direct objects of both these verbs, BrokenThing and OpenedThing; theme
    * prototypically inanimate objects that are affected in some way by the act
  ...
  * modern formulation is due to Fillmore (1968) and Gruber (1965)
...
* The main reason computational systems use semantic roles:
  * shallow meaning representation -> simple inferences that
    * not possible from the pure surface string, or even from the parse tree.
  * QA, MT
* Semantic roles thus help generalize over different surface realizations
  * eg AGENT is often realized as the subject of the sentence,
    in other cases the THEME can be the subject
  * eg _break_
...
* many verbs allow their thematic roles to be realized in various syntactic posz
  * eg verbs like _give_ can realize the THEME and GOAL arguments in two ways:
...
* Levin (1993) lists for
  ...
  * various alternations in which they participate
...
* alternative semantic role models that use either many fewer or many more roles
  * generalized semantic roles that abstract over the specific thematic roles
    ...
    * agent-like properties (volitionally involved in the event,
...
* sentences annotated with semantic roles. The
  * English PropBank labels all the sentences in the Penn TreeBank
  * Chinese PropBank labels sentences in the Penn Chinese TreeBank
...
* non-numbered arguments called ArgMs, (ArgM-TMP, ArgM-LOC, etc.)
  * modification or adjunct meanings
  * relatively stable across predicates, so not listed with each frame file
...
* make inferences in many more situations, across different verbs, and also
  ...
  * the second example uses the different verb rise, and the
    third example uses the noun rather than the verb rise
* The FrameNet project is another semantic-role-labeling project
  ...
  * There are many individual lexical relations of hyponymy, synonymy, and so on
    between many of the words in this list
  * hE these relations do not add up to a complete account of how these words
    are related
  ...
  * frame = the holistic background knowledge that unites these words
    (Fillmore, 1985)
...
* A frame in FrameNet is a background knowledge structure that defines
  * frame elements: a set of frame-specific semantic roles
...
> point (Final value)
* semantic roles (frame elements) in the frame are defined as in Fig. 19.3
...
* codes relationships between frames,
  ...
  * or eg causation
  * generalizations among frame elements in different frames can be represented
    by inheritance as well
  * Thus, there is a `Cause change of position on a scale` frame that is
    linked to the `Change of position on a scale` frame by the cause relation
...
* Feature-based algorithms
  * from the very earliest systems like (Simmons, 1973)—
  * begin by parsing, using broad-coverage parsers to assign a parse
...
* The classification algorithm above classifies each argument separately (‘loc')
  * simplifying assumption; each argument of a predicate labeled independently
    * This assumption is false; there are interactions between arguments
    * eg constituents in FrameNet and PropBank are non-overlapping
    * PropBank does not allow multiple identical arguments; two constituents of
* add a fourth step to deal with global consistency across the labels
...
* Most systems use some generalization of the core set of features introduced by
  ...
  * The headword of the constituent, _Examiner_
    * can be computed with standard head rules, eg those given in Chapter 12
    * Certain headwords (e.g., pronouns) place strong constraints on their role
  * The headword part of speech of the constituent, NNP
  * The path in the parse tree from the constituent to the predicate
    * Gildea and Jurafsky (2000): a simple linear repr of the path, NP↑S↓VP↓VBD
    * a compact representation of many kinds of grammatical function relships
      between the constituent and the predicate
...
* as a sequence labeling task like named entity recognition, using the BIO
* Let’s assume that we are given the predicate and
  the task is just detecting and labeling spans
...
* As with all the taggers, the goal is to compute the highest prob tag seq ŷ,
  `ŷ = argmax_{y∈T} P(y|w)`
* a standard algorithm from He+ (2017)
...
* another way to represent the relationship between predicates and arguments
* selectional restriction is
  a semantic type constraint that a verb imposes on the kind of concepts that
  are allowed to fill its argument roles. Consider the two meanings of
  (19.29) I want to eat someplace nearby
  * two possible parses and semantic interpretations for this sentence
    ...
    * speaker-as-Godzilla interpretation, eat is transitive and
      _someplace nearby_ is the direct object and the THEME of the eating
...
* Selectional restrictions vary widely in their specificity
  eg _imagine_:
    ...
    * very few semantic requirements on its THEME role
  * diagonalize, on the other hand, places a
    * very specific constraint on the filler of its THEME role: it has to be mx
  * _odorless_ are restricted to concepts that could possess an odor: (19.33)
...
* To stipulate the selectional restriction that y must be something edible, we
  * add a new term to that effect: ∃e, x, y Eating(e) ∧ ... ∧ EdibleThing(y)
...
* two problems with its direct use
  ...
  * presupposes a large, logical knowledge base of facts about the concepts
    * such common-sense knowledge bases are being developed, but
...
* eg _ate a hamburger_ example, for instance, we could set
  * THEME role of the verb eat to the synset {food, nutrient}
  * chain of hypernyms shown in Fig. 19.7 reveals that hamburgers are food
* We can apply this approach to the THEME roles of the verbs imagine, or lift
  * imagine’s THEME to the synset {entity}, lift’s THEME to {physical entity},
  * diagonalize to {matrix}
...
* hE Very quickly it became clear that these
...
* One of the most influential models for ~ is by Resnik (1993)
  ...
  * eg _eat_ tells us a lot about the semantic class of its direct objects,
    * _be_, by contrast, tells us less about its direct objects
...
* An even simpler approach is to use the simple
  ...
  log count(v, n, r) instead of conditional probability
...
* pseudowords (Gale+ 1992, Schütze 1992)
...
* alter: human preferences for a test set of verb-argument pairs, and
  have them rate their degree of plausibility
...
* More radical approaches choose to break down _open_ as well
...
* Fillmore was also influenced by the
  semantic field theorists and by a visit to the Yale AI lab where
  ...
  * In the 1990s Fillmore drew on these insights to begin the FrameNet corpus
    annotation project.  At the same time
...
* unsupervised approaches for semantic role labeling attempt to
  induce the set of semantic roles by clustering over arguments
