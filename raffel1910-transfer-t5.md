...
* SQuAD (Rajpurkar+ 2016) is a common question-answering benchmark. In our
  experiments, the model is fed the question and its context and asked to
  generate the answer token-by-token. For 
* WMT English 
...
* text-to-text framework obtained comparable performance to task-specific
  architectures and SOTA results when combined with scale
* Architectures
...
### The inconvenience of large models An unsurprising but important result from

* larger models tend to perform better. The fact that the 
  * hardware is continually getting cheaper and more powerful suggests that
...
* applications and scenarios where smaller or less expensive model is helpful,
  ...
  * low-resource tasks.  Low-resource tasks often occur (by definition) in
* methods that achieve stronger performance with cheaper models so that transfer
  * distillation (Hinton+ 2015; Sanh+ 2019; Jiao+ 2019), 
  * parameter sharing (Lan+ 2019), and 
  * conditional computation (Shazeer+ 2017).
    * activating only some of the units in a network, in an input-dependent
    * e.g. if we think we're looking at a car, we only need to compute the
      activations of the vehicle detecting units, not of all features that a

### More efficient knowledge extraction Recall that 
...
### Formalizing the similarity between tasks We observed that 

* pre-training on unlabeled in-domain data can improve performance (Section 3.4)
* relies on e.g. that SQuAD was created using data from Wikipedia. It would be
* formulate a more rigorous notion of the “similarity”
* in computer vision (Huh+ 2016; Kornblith+ 2018; He+ 2018). A better notion of
* could also help choose supervised pre-training tasks, which 
  has been shown to be helpful for the GLUE benchmark (Phang+ 2018).

### Language-agnostic models We were disappointed to find that 
