...
* SarcNet, a multilingual and multimodal sarcasm detection dataset in
  ...
  * facilitates a more accurate and reasonable assessment of uni and multimodal
* 10,000 labeled instances

# 1 Intro

* Sarcasm: to mock, insult, or convey contempt 
  by expressing the opposite of what is meant
...
* dataset quality,
  * a majority of these studies have relied on a single dataset for
    * potentially inconclusive findings
  ...
  * shortcomings in the labeling method. The existing labeling method only
    defines a unified label for a multimodal instance
...
* SarcNet, a new multilingual and multimodal sarcasm detection dataset,
  ...
  * substantial or higher levels of agreement in the annotation tasks, as
    * Cohen’s Kappa values of 0.9032, 0.7129, and 0.9769 
      for textual, visual, and multimodal labels, respectively
* benchmark models: Well-known unimodal and multimodal
  ...
  * text data were evaluated using CNN, RNN, and Transformers
    * Transformer-based models such as XLM-RoBERTa, endowed with more
      parameters, achieved superior accuracy (76.88%) and F1-score (73.36%)
  * Multimodal models facilitate enhanced integration of image and text,
    resulting in improved performance, with
    DT4MID leveraging XLM-RoBERTa attaining the best results 
    (accuracy of 82.73% and F1-score of 84.40%)
...
* in social media platforms, text often coexists with images
  ...
  * Cai+ (2019) presented a hierarchical fusion model
* models capable of processing both text and images, such as
  * eg CLIP (Radford+ 2021) and BLIP-2 (Li+ 2023), has provided more
    ...
    complementary nature of information in visual and textual data
...
* attention mechanisms and fusion strategies has been instrumental
  * Attention mechanisms allow models to
    weigh the importance of different modalities, while
  ...
  * Pan+ (2020): a BERT-based model, incorporating inter-modality attention to
    * discern both intra and inter-modality incongruities
  ...
  * DT4MID (Tomás+ 2023) integrated textual and visual transformers within a
    deep neural network framework to achieve superior multimodal integration
  * Malik+ (2023) conducted an analysis to ascertain the 
    necessity of image information in comprehending the sarcastic intent
...
* Most current multimodal sarcasm detection models (Pan+ 2020; Wang+ 2020;
  Liang+ 2021; Liu+ 2022) are tested using only one dataset
...
* there are a few cases with
  ...
  * {1, 0, 0}: When there exists information about sarcasm in the image
    ...
    * eg “I asked MemeCreator to generate a sarcastic image” (non-sarcastic
      text) with a sarcastic image
  * {0, 1, 0} labels are typically due to the
    very weak connections between text and images
    * their combinations cannot be identified as sarcasm
