...
* Our model
  * combines masked language model (MLM) and translation language model (TLM)
    pretraining with a translation ranking task using bi-directional dual
    encoders
...
* pretrained MLMs do not intrinsically produce good sentence-level embeddings.
  ...
  * SentenceBERT (Reimers and Gurevych, 2019) fine-tunes a monolingual BERT
    based dual-encoder on natural language inference (NLI). The resulting
...
* multilingual sentence embedding models incorporate similar dual encoders, 
  * none explore MLM pretraining. Rather, encoders are 
  * trained on translation pairs 
    * directly (Artetxe and Schwenk, 2019b; Guo+ 2018; Yang+ 2019a), or on
      * require large amounts of parallel training data.
    * combined with monolingual input-response prediction (Chidambaram+ 2019;
      Yang+ 2019b). Multilingual sentence embeddings trained 
* multilingual universal sentence encoder (m-USE) that are trained on multiple
  languages often perform worse than similar models only targeting a single
  language pair (Yang+ 2019a).
...
* we present a novel method for training multilingual sentence-level embeddings
  * combining existing SOTA methods for multilingual sentence embeddings with
    ...
    pretrained encoders. We employ a 
  * dual-encoder framework which consist of paired encoders feeding a
    combination function.  Such models are well suited for learning effective
    cross-lingual sentence embeddings using an additive margin softmax loss
    (Yang+ 2019a). Source and target sentences are encoded separately using a
    shared BERT based encoder (Devlin+ 2019). The final layer [CLS]
    representations are taken as the sentence embeddings for each input. The
    similarity between the source and target sentences is scored using cosine
    over the sentence embeddings produced by the BERT encoders. The model
    architecture is illustrated in figure 1.  
* outperforms the previous SOTA, which are mostly bilingual models, on large
  bitext mining tasks e.g. the United Nations corpus (Ziemski+ 2016) and BUCC
  (Zweigenbaum+ 2018), containing pools with tens of millions and hundreds of
...
* contributions of this paper are: 
  * A combination of pre-training and finetuning strategies to boost the
    performance of a dual encoder translation ranking model to the SOTA
    performance on bi-text mining; 
  * A single massively multilingual model spanning 109 languages and showing
    cross-lingual transfer even to zeroshot cases. 
  * A thorough analysis and ablation study to understand the impact of various
...
* conclusion
  ...
  * the amount parallel data required could be largely diminished with the
    masked language model pre-training. The pre-trained model is released at

# 5 Analysis

### Additive Margin 

* The additive margin (Yang+ 2019a) is still an very important part for learning
...
* initializing from multilingual BERT model 12 . This model 
...
* In order to understand how the data selection affect the model performance, 
  * we also train a model with the original web crawled translation pairs
    ...
    NMT models.  Surprisingly, this model 
...
* total of 30+ such languages 13 . The 
* performance is surprisingly good for most of the languages with an 
  ...
  * We analyze effect of vocabulary by inspecting the 
    ...
    * Languages with low performance tend to have shorter token lengths and
      longer sequence lengths indicating which indicates sub-par vocab coverage
    * A better vocab could potentially benefit those languages.

### Negative Sampling, cross-accelerator negative sampling

* We also briefly explore using hard rather than random negatives.  Results are
* Guo+ (2018) explored hard negative mining in the dual encoder framework for
  learning crosslingual embeddings, and this technology has been used as the
...
* We experiment the hard negative mining for Spanish (es) following Guo+ (2018)
  ...
  * For languages other than `es`, batch size was decreased to 32 together.  
* Removing the cross-accelerator sampling hurts the model performance slightly
  * hard negatives don't help the LaBSE model’s performance.  The P@1 for es is
...
* the English Semantic Textual Similarity (STS) benchmark (Cer+ 2017) 
  ...
  * broadly used to evaluate the quality of sentence-level embeddings by
* distance measures
  * Within prior work, 
    * m-USE, USE and ConvEmbed use arccos distance to measure embedding space
    * InferSent and SentenceBERT use cosine similarity.  Table 7 reports
    * ours is computed as the arccos distance between the pair’s sentence
      embeddings. 14 In addition to performance using similarity scores commuted
    * we include numbers for SentenceBERT when it is fine-tuned for the STS task
      ...
      the embeddings to STS. Rather than directly measuring sentence embeddings
...
* We observe that LaBSE performs 
  * worse on pairwise English semantic similarity than other sentence embedding
  * excellent performance on crosslingual bi-text retrieval. The cross-lingual
