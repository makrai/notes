...
* This fact has given rise to a number of philosophical problems; I discuss 3
  ...
  * the pragmatic problem of counterfactuals
    * has dominated recent discussions of contrary-to-fact conditionals is
    * the formal properties of the conditional function, together with all of
      the facts, may not be sufficient for determining the truth value of a
    * ie different truth values of conditional statements may be consistent
      with a single valuation of all nonconditional statements. The task set by
    ...
    * pragmatic, and not semantic
      ...
      * Rescher (1964) avoids this boundary line by couching his
        ...
        rather than truth
      * Conditions for justified belief are pragmatic for any concept. 2
  * an epistemological problem that has bothered empiricist philosophers. It is
    * many counterfactuals seem to be synthetic, and contingent, statements
    ...
    * philosophers to try to analyze the conditional in non-conditional terms 3
      to show that conditionals merely appear to be about unrealized possibilits
      * endnote; Chisholm (1946), 289-307,
        ...
        * it is a mistake to base very much on the distinction of mood. As far
...
* eg situation: you are faced with a true-false political opinion survey. The
  ...
  * two familiar answers to this question, and then defend a third answer which
  1. based on the simplest account of the conditional, the truth functional anal
    ...
    * If the answer to the first question is no,
      or if the answer to the second is yes, then => 'true' box. But this
    * unacceptable since the following piece of reasoning is an obvious non
      * for even an indicative conditional.
  2. a conditional affirms some sort of logical or causal connection
    ...
    * If the hypothesis were accepted, then task; clarifying 'connection'
    * but there are counter-examples even with this notion left as obscure as it
    * eg you
      ...
      * you have no opinion about the statement, "The Chinese will enter"
        ...
        * What happens to the idea of connection on this hypothesis?
          ...
          * if you already believe the consequent (even if believe it to be
            ...
            your stock of beliefs when you add the antecedent, since the rational
            man does not change his beliefs without reason.
          * In either case, you will affirm the conditional. Thus this answer
            accounts for the relevance of 'connection' when it is relevant without
            making it a necessary condition of the truth of a conditional.
    * you believe or know the antecedent to be true; in this case,
      * no changes need be made in your stock of beliefs. If you already believe
      * your belief about the conditional will be just the same as your belief
        about the statement that the U.S. will use the bomb.
    * you know or believe the antecedent to be false? In this situation,
      * you cannot simply add it to your stock of beliefs
        without introducing a contradiction. You must
      * make adjustments by deleting or changing those beliefs which conflict
        with the antecedent
      * difficulties; there will be more than one way to make the adjustments
        * Rescher calls this the problem of the ambiguity of belief-contravening
          hypotheses.
...
* It is crucial, however, that the answer may not be restricted to some
  ...
  * If the conditional is to be understood as a function of the
    ...
    its truth value should not in general be dependent on the attitudes
...
* The concept of a possible world is just what we need to make this
  * the ontological analogue of a stock of hypothetical beliefs. The following
  ...
  * Consider a possible world in which A is true, and which otherwise differs
    ...
    B is true (false) in that possible world.
  * advantage of providing a ready made apparatus on which to build a formal
    semantical theory
...
* first define a model structure. Let M be an ordered triple (K, R, \lambda).
  ...
  * R is the relation of relative possibility which defines the structure.
    If a and \beta are possible worlds (members of K), then
    a R \beta reads "\beta is possible with respect to a". This means that,
    where a is the actual world, \beta is a possible world
    R is a reflexive relation; that is, every world is possible with respect
    * you may add that R must be transitive, or transitive and symmetrical.?
      * endnote;
      * we build on is von Wright's M. If we add the
      * transitivity requirement => Lewis's S4
      * both the transitivity and symmetry requirements, then S5
  * \lambda
    * The only element that is not a part of the standard modal semantics is
  ...
  * The purpose of A is to allow for an interpretation of "If A, then B" in the
    case where A is impossible; for this situation one needs an impossible
...
* required that the world selected differ __minimally__ from the actual world. This
...
* These conditions on the selection function don't determine the function unique
  * There may be further formal constraints that can plausibly be imposed on the
  * not expect to find semantic conditions sufficient to guarantee uniqueness
  * The questions ~ are reformulations of the pragmatic problem of
    * "On what basis do we select a selection function from among the acceptable
    * "What are the criteria for ordering possible worlds?" are .
...
* a formal system, C2
  ...
  * Other modal and conditional concepts can be defmed in terms of the corner as

□A = ~A > A
⋄A = ~(A > ~A)
A ≷ B = (A > B) & (B > A)
* The rules of inference of C2 are
  * modus ponens (if A and A \supset B are theorems, then B is a theorem) and
...
* The corner
  ...
  * these differences help to explain some peculiarities of counterfactuals. I
  * three unusual features of the conditional connective.
  (1) Unlike both material and strict implication,
    ...
    * That is, from A > B and B > C, one cannot infer A > C. While this may at
    * eg Premisses
        ...
        communist."
      Conclusion
        "If J. Edgar Hoover had been born a Russian, he would be a traitor." It
  ...
  (3) The inference of contraposition,
    valid for both the truth-functional horseshoe and the strict implication
    hook, is
...
* Examples of these anomalies have been noted by philosophers in the past.
  ...
  * "full counterfactuals affirm, while semifactuals deny, that a certain
    connection obtains between antecedent and consequent
    * The practical import of a semifactual is thus different from its literal
      import."
  ...
  * A separate and nonconditional analysis for semifactuals is necessary to save
    the 'connection' theory of counterfactuals in the face of the anomalies we
* summ
  * The theory presented in Section II avoids this difficulty
    by denying that the conditional are, in general, to assert a connection of
  * It is the structure of inductive relations and causal connections which
    make counterfactuals and semifactuals true or false, but
